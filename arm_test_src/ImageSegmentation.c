...
INT32T ComputeDataCost(INT32T idx, INT32T l){
	INT32T gray, cost;
	gray = RGB2Gray(image_Correction[idx]);
	switch (l)
	{
	case 0: {cost = max2(gray - 10, 0) + 80; break;}
	case 1: {cost = max2(210 - gray, 0) + 10; break;}
	case 2: {if (image_Canny[idx] > 0) cost = 1; else cost = max2(max3(image_Correction[idx].r, image_Correction[idx].g, image_Correction[idx].b), max3(255 - image_Correction[idx].r, 255 - image_Correction[idx].g, 255 - image_Correction[idx].b)) * 7 / 10;  break; }
	default: {cost = 0; break; }
	}
	//printf("(r=%d g=%d b=%d gray=%d) l=%d cost=%d\n", image_Correction[idx].r, image_Correction[idx].g, image_Correction[idx].b, gray, l, cost);
	return cost;
}
